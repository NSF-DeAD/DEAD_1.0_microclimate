...
combined_kestrel_notcleaned <- map_dfr(allfiles, safe_read_fun)

# Remove NULL results (if any files failed to read)
combined_kestrel_notcleaned <- combined_kestrel_notcleaned %>% filter(complete.cases(.))
...
combined_kestrel_cleaning$datetime <- NA

# Try parsing with flexible parse_date_time function
combined_kestrel_cleaning$datetime <- parse_date_time(
  combined_kestrel_cleaning$loggertime, 
...
if (any(is.na(combined_kestrel_cleaning$datetime))) {
  cat("Some entries could not be parsed!\n")
  # Print rows where the time column is still NA
  print(combined_kestrel_cleaning[is.na(combined_kestrel_cleaning$datetime),])
}
...
combined_kestrel_cleaning <- combined_kestrel_cleaning[!is.na(combined_kestrel_cleaning$datetime), ]

# Ensure the time is in 24-hour format (POSIXct will handle it correctly)
combined_kestrel_cleaning$datetime <- as.POSIXct(combined_kestrel_cleaning$datetime, format = "%Y-%m-%d %H:%M:%S")

# Round down to the nearest minute (sets seconds to :00)
combined_kestrel_cleaning$datetime <- floor_date(combined_kestrel_cleaning$datetime, unit = "minute")
...
combined_kestrel_cleaning <- combined_kestrel_cleaning |>
  distinct()
